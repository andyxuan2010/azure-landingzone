...
> This document is a broad comparison of delivery systems. For the exact four values accepted by `app_services[*].deployment_method`, their App Service settings, and current sibling GitHub Actions compatibility, use [APP_SERVICE_DEPLOYMENT_METHODS.md](./APP_SERVICE_DEPLOYMENT_METHODS.md).
...
- A site can appear healthy at the App Service level while still serving only the default Azure landing page if Deployment Center never actually synced content into the app.
...
One practical Deployment Center failure mode is:

- App Service state is `Running`
- the runtime stack looks correct
- but the public site serves the standard Azure placeholder page instead of the application

That usually means the Deployment Center repo connection exists in configuration, but no successful content deployment actually landed. A quick signal is an empty result from `az webapp log deployment list` for the target app.
