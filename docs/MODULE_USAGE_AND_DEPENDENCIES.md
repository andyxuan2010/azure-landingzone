...
In the root landing zone, use the per-app `deployment_method` selector in `app_services` as the single source of truth. Supported values are `external_zip_deploy`, `run_from_package`, `deployment_center`, and `zip_deploy_with_build`. The legacy `deployment_center_enabled` flag is backward-compatible only and should not be mixed with another selected method. If you choose Deployment Center with Azure Repos, pass the organization, project, repository, and branch together so the repo URL is derived once in Terraform instead of being copied across pipelines or app settings.
`external_zip_deploy` and `run_from_package` expect a prebuilt application artifact; the latter mounts the artifact with `WEBSITE_RUN_FROM_PACKAGE=1`. `zip_deploy_with_build` uploads source and enables Kudu/Oryx remote build. `deployment_center` binds App Service directly to a repository. See [APP_SERVICE_DEPLOYMENT_METHODS.md](./APP_SERVICE_DEPLOYMENT_METHODS.md) for the complete contract.
