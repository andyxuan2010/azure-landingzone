...
6. Run `terraform init -backend=false` and `terraform validate` in the module directory before changing tests or examples.
7. Treat `tests/live.tftest.hcl` as environment-mutating live tests. Review names, credentials, and target resource groups before running `terraform test`.
...
| `appservice` | Linux or Windows Web App | Existing resource group; existing App Service Plan | Subnet for VNet integration, private endpoint subnet, private DNS zone, Entra app registration for auth, Log Analytics workspace, Azure DevOps repo for Deployment Center | `modules/appservice/examples/` |
...
In the root landing zone, use the per-app `deployment_method` selector in `app_services` as the single source of truth to choose between `run_from_package`, `deployment_center`, and `zip_deploy_with_build`. The legacy `deployment_center_enabled` flag is backward-compatible only and should not be mixed with `deployment_method = "run_from_package"`. If you choose Deployment Center with Azure Repos, pass the organization, project, repository, and branch together so the repo URL is derived once in Terraform instead of being copied across pipelines or app settings.
There is also a third App Service deployment pattern used by some sibling application repos: CI-driven ZIP deploy with remote build automation. In that pattern, the pipeline uploads a ZIP with `az webapp deploy` and enables `SCM_DO_BUILD_DURING_DEPLOYMENT=true` (and sometimes `ENABLE_ORYX_BUILD=true`) so Kudu/App Service performs the build on the server. This is distinct from both Deployment Center and true run-from-package, and it is now exposed as `deployment_method = "zip_deploy_with_build"` in the root landing zone.
When private endpoint plus private DNS is enabled for App Service, make sure the deployment runner has both network reachability to the private App Service path and Azure RBAC on the target web app. In this landing zone, the root module grants the Linux runner VM system-assigned identity `Website Contributor` on each enabled App Service so deployments can run with `az login --identity`.
In this landing zone, enabling `app_service_enable_private_endpoint` is not enough by itself. The current root wiring passes `private_dns_zone_name` and `private_dns_zone_resource_group_name` to the downstream module, and that module uses `data.azurerm_private_dns_zone` to look up an existing zone when no explicit zone ID is supplied. That means the environment must either:
1. include `privatelink.azurewebsites.net` in `private_dns_zone_names` so the root `private_dns` module creates it, or
2. point to an already-existing zone with the correct ID or correct name/resource-group pair.
If neither is true, Terraform will fail with `Private Dns Zone ... was not found` even though private endpoint creation is enabled.
Free (`F1`) and Shared (`D1`) App Service plans do not support private endpoints in the root landing zone wiring, so keep private endpoint settings disabled for those SKUs or move the workload to `B1` or higher first.
...
In the root landing zone, the base Automation Account is configured through `automation_accounts`, and Azure Resource Inventory is exposed as a separate workload layer through `automation_ari_workloads`. That split keeps the account reusable while still allowing the landing zone to provision the ARI runtime environment, package imports, runbook, blob container, and schedule when needed.
The current ARI workload layer also includes runbook timing logs, optional low-cost execution toggles such as `ari_lite_mode`, and Azure Monitor scheduled query alerts for failed and long-running jobs.
For the current ARI implementation details and example inputs, see [AUTOMATION_ARI.md](./AUTOMATION_ARI.md).
...
For dev/test environments that should use the Azure SQL free monthly offer, set the database to General Purpose serverless (`sql_sku_name = "GP_S_Gen5_2"`), keep `sql_max_size_gb = 32`, set `sql_use_free_limit = true`, and choose `sql_free_limit_exhaustion_behavior = "AutoPause"` unless billable overage is intentional. The root module passes these values through to the template `sqldb` module, including `sql_auto_pause_delay_in_minutes`, `sql_min_capacity`, and `sql_geo_backup_enabled`.

Azure does not support converting an existing paid Azure SQL database into the free offer in place. If the database already exists as paid, keep `sql_use_free_limit = false` or recreate the database intentionally before enabling the free limit. Sandbox follows this rule to avoid `ProvisioningDisabled: Cannot update paid database to free database`.
...
- Every module currently has a `tests/live.tftest.hcl` file.
- The `appservice` live test now creates its own temporary App Service Plan fixture instead of relying on a hard-coded external plan.
- The `functionapp` live test already uses the same pattern and remains the reference for dependent-module live tests.
- Several other live tests still target shared long-lived environment resources by design. Keep those tests for integration validation, but prefer fixture modules for any new dependencies you add.
