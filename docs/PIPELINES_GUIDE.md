...
3. `quality-gate`
4. `create-git-tag`
5. `publish-stage-repo`
6. `publish-ado-repo`

There are also disabled placeholder apply jobs in the file for `dev`, `sandbox`, and `prod`, but they are all currently gated off with `if: false`.
...
- runs `quality-gate`, which currently verifies only that `validate-plan` completed successfully
