...
| Azure DevOps | `azure-pipelines.yml` | Run Terraform validate, plan, artifact publish, and apply in Azure DevOps | Triggered on `main` in the checked-in YAML | Apply stage is enabled with `condition: true` |

## GitHub Actions Pipeline

### What it does

The GitHub Actions workflow is currently built around these active jobs:
...
6. `apply_sandbox`
7. `create-git-tag`
8. `publish-stage-repo`
9. `publish-ado-dev-repo`
10. `publish-ado-sandbox-repo`
...
- checks out the landing zone repo and clones the public template repo from `GH_TF_TEMPLATE_REPO` into `../template`
- configures Git authentication for any remaining Azure DevOps module downloads
- runs separate plan jobs for `dev` and `sandbox` when `DEPLOY_DEV` or `DEPLOY_SANDBOX` is enabled
- runs Trivy HIGH/CRITICAL IaC misconfiguration and repository-secret scanning unless `ENABLE_IAC_SECRET_SCAN` is exactly `false`
- runs:
...
- warns and runs `terraform fmt -recursive` on a best-effort basis instead of failing when formatting drift is detected
- can run apply jobs after a successful plan when apply inputs are enabled
...
GitHub Actions also uses repo variables such as:

- `GH_TF_TEMPLATE_REPO`
- `DEPLOY_DEV`
- `DEPLOY_SANDBOX`
- `ENABLE_GITHUB_APPLY`
- `ENABLE_IAC_SECRET_SCAN`

The ARM secrets are GitHub Environment secrets used for Terraform provider authentication. `GH_TF_TEMPLATE_REPO` points at the public GitHub template repo, for example `CCOE-Azure-Terraform/azure-template`. `AZURE_ADO_PAT2` is retained for any Terraform module sources that still resolve through Azure DevOps. The Trivy scan is enabled by default; set `ENABLE_IAC_SECRET_SCAN` to exactly `false` only when a temporary, approved exception is required.
...
- After a successful validation/plan run, it creates a canonical GitHub release tag and publishes a clean snapshot of the repo to:
  a GitHub stage repo
...
- It runs Trivy in the static validation job, but does not currently run separate `tflint` or `terraform-docs` jobs in GitHub Actions.
- The checked-in `apply` jobs are intentionally disabled.
...
- uploads compact Terraform plan/apply summaries to the Azure DevOps run summary
...
- If the `INFRACOST_API_KEY` GitHub Actions secret is configured, GitHub Actions also generates an Infracost estimate from the Terraform plan, adds a detailed table to the job summary, uploads the Infracost JSON and text reports as workflow artifacts, and posts or updates a PR comment on pull requests.
...
- Azure DevOps does not perform repo snapshot publishing or GitHub release tag creation.
...
- The GitHub Actions workflow can apply after a successful plan when apply inputs are enabled.
- GitHub Actions now creates the canonical GitHub release tag before publishing snapshots downstream.
