...
5. `apply_sandbox`
6. `create-git-tag`
7. `publish-stage-repo`
8. `publish-ado-dev-repo`
9. `publish-ado-sandbox-repo`
...
The ARM secrets are GitHub Environment secrets used for Terraform provider authentication. `GH_TF_TEMPLATE_REPO` points at the public GitHub template repo, for example `CCOE-Azure-Terraform/azure-template`. `AZURE_ADO_PAT2` is retained for any Terraform module sources that still resolve through Azure DevOps.
...
- It does not currently run separate `tflint`, `trivy`, or `terraform-docs` jobs in GitHub Actions.
