...
| Name | Source | Version |
|------|--------|---------|
| acr | ../azure-template/modules/acr | n/a |
| adf_basic | ../azure-template/modules/adf | n/a |
| aks | ../azure-template/modules/aks | n/a |
| app_registration_appservice | ../azure-template/modules/appregistration | n/a |
| app_service | ../azure-template/modules/appservice | n/a |
| app_service_plan | ../azure-template/modules/appserviceplan | n/a |
| automation_account | ../azure-template/modules/automationaccount | n/a |
| azure_ai_search | ../azure-template/modules/azure_ai_search | n/a |
| azure_ai_service | ../azure-template/modules/azure_ai_service | n/a |
| databricks | ../azure-template/modules/databricks | n/a |
| fortigate | ../azure-template/modules/fortigate | n/a |
| hub_virtual_network | ../azure-template/modules/vnet | n/a |
| keyvault | ../azure-template/modules/keyvault | n/a |
| linux_vm_basic | ../azure-template/modules/linuxvm | n/a |
| log_analytics | ../azure-template/modules/loganalytics | n/a |
| mg_landingzone | ../azure-template/modules/managementgroups | n/a |
| mg_landingzone_children | ../azure-template/modules/managementgroups | n/a |
| mg_platform | ../azure-template/modules/managementgroups | n/a |
| mg_platform_children | ../azure-template/modules/managementgroups | n/a |
| mg_sandboxes | ../azure-template/modules/managementgroups | n/a |
| network_security_group | ../azure-template/modules/nsg | n/a |
| openai | ../azure-template/modules/openai | n/a |
| private_dns | ../azure-template/modules/private_dns | n/a |
| resource_group | ../azure-template/modules/rg | n/a |
| spoke_virtual_network | ../azure-template/modules/vnet | n/a |
| sqldb | ../azure-template/modules/sqldb | n/a |
| storage_account | ../azure-template/modules/storageaccount | n/a |
| subscription_vending | ../azure-template/modules/subscription_vending | n/a |
| winvm_basic | ../azure-template/modules/winvm | n/a |
...
| adf_enable_diagnostics | Whether to send Azure Data Factory diagnostics to the shared Log Analytics workspace. | `bool` | `false` | no |
...
| app_service_app_secret_key | Application secret key setting for the sample app. Replace before production use. | `string` | `"replace-with-a-random-secret-32-long"` | no |
...
| connectivity_tenant_id | Azure tenant ID used by the aliased azurerm.connectivity provider. | `string` | `"d5b038fb-4b39-41cc-8a10-fba75212180b"` | no |
| databricks_access_connector_id | Optional existing Databricks access connector resource ID. | `string` | `""` | no |
| databricks_access_connector_identity_ids | User-assigned identity IDs for the Databricks access connector. | `list(string)` | `[]` | no |
| databricks_access_connector_name | Optional Databricks access connector name. | `string` | `""` | no |
| databricks_access_connector_role_assignments | Role assignments for the Databricks access connector identity. | `any` | `{}` | no |
| databricks_access_connector_system_assigned_identity_enabled | Whether the Databricks access connector uses a system-assigned identity. | `bool` | `true` | no |
| databricks_create_access_connector | Whether to create a Databricks access connector. | `bool` | `false` | no |
| databricks_custom_parameters | Optional Databricks custom parameters for VNet injection, no-public-IP, storage, and ML linkage. | `any` | `null` | no |
| databricks_customer_managed_key_enabled | Whether customer-managed keys are enabled for Databricks. | `bool` | `false` | no |
| databricks_default_storage_firewall_enabled | Whether the Databricks default storage firewall is enabled. | `bool` | `false` | no |
| databricks_diagnostic_eventhub_authorization_rule_id | Optional Event Hub authorization rule ID for Databricks diagnostics. | `string` | `null` | no |
| databricks_diagnostic_eventhub_name | Optional Event Hub name for Databricks diagnostics. | `string` | `null` | no |
| databricks_diagnostic_setting_name | Optional Databricks diagnostic setting name. | `string` | `""` | no |
| databricks_diagnostic_storage_account_id | Optional Storage Account ID for Databricks diagnostic archive. | `string` | `null` | no |
| databricks_enable_diagnostics | Whether Databricks diagnostic settings are enabled. | `bool` | `false` | no |
| databricks_enhanced_security_compliance | Optional enhanced security and compliance settings for Databricks. | `any` | `null` | no |
| databricks_infrastructure_encryption_enabled | Whether infrastructure encryption is enabled for Databricks. | `bool` | `false` | no |
| databricks_inherit_resource_group_tags | Whether the Databricks module should merge tags from the target resource group. | `bool` | `false` | no |
| databricks_log_analytics_workspace_id | Optional Log Analytics workspace ID for Databricks diagnostics. | `string` | `""` | no |
| databricks_managed_disk_cmk_key_vault_id | Optional Key Vault ID for Databricks managed disk CMK. | `string` | `""` | no |
| databricks_managed_disk_cmk_key_vault_key_id | Optional Key Vault key ID for Databricks managed disk CMK. | `string` | `""` | no |
| databricks_managed_disk_cmk_rotation_to_latest_version_enabled | Whether Databricks managed disk CMK rotates to the latest key version. | `bool` | `false` | no |
| databricks_managed_resource_group_name | Optional managed resource group name for the Databricks workspace. | `string` | `""` | no |
| databricks_managed_services_cmk_key_vault_id | Optional Key Vault ID for Databricks managed services CMK. | `string` | `""` | no |
| databricks_managed_services_cmk_key_vault_key_id | Optional Key Vault key ID for Databricks managed services CMK. | `string` | `""` | no |
| databricks_name | Optional Databricks workspace name override. | `string` | `""` | no |
| databricks_network_security_group_rules_required | Network security group rules mode for Databricks VNet injection. | `string` | `"NoAzureDatabricksRules"` | no |
| databricks_private_dns_zone_ids | Private DNS zone IDs attached to Databricks private endpoints. | `list(string)` | `[]` | no |
| databricks_private_dns_zone_names | Private DNS zone names resolved for Databricks private endpoints. | `list(string)` | `[]` | no |
| databricks_private_dns_zone_resource_group_name | Resource group containing Databricks private DNS zones. | `string` | `null` | no |
| databricks_private_endpoint_manual_connection_enabled | Whether Databricks private endpoints use manual private service connection approval. | `bool` | `false` | no |
| databricks_private_endpoint_network_interface_name | Optional base network interface name for Databricks private endpoints. | `string` | `""` | no |
| databricks_private_endpoint_network_resource_group_name | Network resource group used for Databricks private endpoint lookup when subnet ID is not set. | `string` | `""` | no |
| databricks_private_endpoint_request_message | Manual private endpoint connection request message for Databricks. | `string` | `""` | no |
| databricks_private_endpoint_subnet_id | Optional subnet ID for Databricks private endpoints. Defaults to the landing zone private endpoint subnet. | `string` | `""` | no |
| databricks_private_endpoint_subnet_name | Subnet name used for Databricks private endpoint lookup when subnet ID is not set. | `string` | `""` | no |
| databricks_private_endpoint_subresource_names | Databricks private endpoint subresources, for example databricks_ui_api or browser_authentication. | `list(string)` | `[]` | no |
| databricks_private_endpoint_vnet_name | VNet name used for Databricks private endpoint lookup when subnet ID is not set. | `string` | `""` | no |
...
| databricks_public_network_access_enabled | Whether public network access is enabled for the Databricks workspace. | `bool` | `false` | no |
...
| databricks_role_assignments | Additional Databricks workspace role assignments. | `any` | `{}` | no |
| databricks_root_dbfs_customer_managed_key | Optional customer-managed key configuration for Databricks root DBFS. | `any` | `null` | no |
...
| enable_databricks | Enable the Azure Databricks workspace module in the landing zone. | `bool` | `false` | no |
...
| fortigate_admin_password | Optional FortiGate administrator password. Supply through a secret variable. | `string` | `""` | no |
| fortigate_admin_ssh_public_key | Optional SSH public key for FortiGate administration. When empty, linux_vm_admin_ssh_key is used. | `string` | `""` | no |
| fortigate_admin_ssh_source_address_prefixes | Trusted IPv4 addresses or CIDR prefixes allowed to administer FortiGate over TCP/22. | `list(string)` | `[]` | no |
| fortigate_admin_username | FortiGate VM administrator username. | `string` | `"azureadmin"` | no |
| fortigate_create_dedicated_vnet | Whether the FortiGate module creates a dedicated VNet. When false, it creates the FortiGate subnets in the shared hub VNet. | `bool` | `false` | no |
| fortigate_custom_data | Optional FortiOS bootstrap configuration. | `string` | `""` | no |
| fortigate_dedicated_external_subnet_prefixes | Address prefixes for the external subnet when FortiGate uses a dedicated VNet. | `list(string)` | <pre>[<br>  "10.168.32.0/24"<br>]</pre> | no |
| fortigate_dedicated_internal_subnet_prefixes | Address prefixes for the internal subnet when FortiGate uses a dedicated VNet. | `list(string)` | <pre>[<br>  "10.168.33.0/24"<br>]</pre> | no |
| fortigate_deployment_mode | FortiGate deployment mode. single creates one VM; ha creates two VMs. | `string` | `"single"` | no |
| fortigate_external_subnet_name | Optional name for the FortiGate external subnet in the hub VNet. | `string` | `""` | no |
| fortigate_external_subnet_prefixes | Address prefixes for the FortiGate external subnet in the hub VNet. | `list(string)` | <pre>[<br>  "10.167.34.0/24"<br>]</pre> | no |
| fortigate_image | Marketplace image reference for FortiGate. | <pre>object({<br>    publisher = string<br>    offer     = string<br>    sku       = string<br>    version   = string<br>  })</pre> | <pre>{<br>  "offer": "fortinet_fortigate-vm_v5",<br>  "publisher": "fortinet",<br>  "sku": "fortinet_fg-vm",<br>  "version": "latest"<br>}</pre> | no |
| fortigate_internal_subnet_name | Optional name for the FortiGate internal subnet in the hub VNet. | `string` | `""` | no |
| fortigate_internal_subnet_prefixes | Address prefixes for the FortiGate internal subnet in the hub VNet. | `list(string)` | <pre>[<br>  "10.167.35.0/24"<br>]</pre> | no |
| fortigate_license_type | FortiGate licensing model marker. The Marketplace image and plan must match this value. | `string` | `"byol"` | no |
| fortigate_management_access_model | Private management access model recorded on FortiGate resources. | `string` | `"private"` | no |
| fortigate_marketplace_plan | Marketplace plan for the selected FortiGate image. | <pre>object({<br>    name      = string<br>    product   = string<br>    publisher = string<br>  })</pre> | <pre>{<br>  "name": "fortinet_fg-vm",<br>  "product": "fortinet_fortigate-vm_v5",<br>  "publisher": "fortinet"<br>}</pre> | no |
| fortigate_name_prefix | Optional name prefix for FortiGate VM and NIC resources. | `string` | `""` | no |
| fortigate_network_interfaces | Optional FortiGate NIC definitions. Empty uses external and internal NICs with static .4 addresses. | <pre>list(object({<br>    name                           = string<br>    role                           = string<br>    subnet_name                    = string<br>    address_prefixes               = optional(list(string), [])<br>    primary                        = optional(bool, false)<br>    enabled_in_modes               = optional(set(string), ["single", "ha"])<br>    private_ip_address_allocation  = optional(string, "Dynamic")<br>    private_ip_addresses_by_suffix = optional(map(string), {})<br>    enable_ip_forwarding           = optional(bool, true)<br>    accelerated_networking_enabled = optional(bool, false)<br>  }))</pre> | `[]` | no |
| fortigate_network_security_group_name | Optional name for the NSG associated with FortiGate subnets. | `string` | `""` | no |
| fortigate_network_security_rules | NSG rules applied to the FortiGate subnets. | <pre>map(object({<br>    priority                     = number<br>    direction                    = string<br>    access                       = string<br>    protocol                     = string<br>    source_port_range            = optional(string)<br>    source_port_ranges           = optional(list(string))<br>    destination_port_range       = optional(string)<br>    destination_port_ranges      = optional(list(string))<br>    source_address_prefix        = optional(string)<br>    source_address_prefixes      = optional(list(string))<br>    destination_address_prefix   = optional(string)<br>    destination_address_prefixes = optional(list(string))<br>    description                  = optional(string)<br>  }))</pre> | `{}` | no |
| fortigate_os_disk | OS disk settings for FortiGate VMs. | <pre>object({<br>    caching              = optional(string, "ReadWrite")<br>    storage_account_type = optional(string, "Premium_LRS")<br>    disk_size_gb         = optional(number)<br>  })</pre> | `{}` | no |
| fortigate_resource_group_name | Optional existing resource group for FortiGate compute resources. Defaults to the landing zone resource group. | `string` | `""` | no |
| fortigate_virtual_network_address_space | Address space for a dedicated FortiGate VNet. The FortiGate subnet prefixes must fall within this space. | `list(string)` | <pre>[<br>  "10.168.32.0/22"<br>]</pre> | no |
| fortigate_virtual_network_name | Optional dedicated FortiGate VNet name. Used only when fortigate_create_dedicated_vnet is true. | `string` | `""` | no |
| fortigate_vm_size | Azure VM size for FortiGate instances. | `string` | `"Standard_F4s_v2"` | no |
| fortigate_zone | Optional availability zone for a single FortiGate VM. | `string` | `""` | no |
...
| identity_tenant_id | Azure tenant ID used by the aliased azurerm.identity provider. | `string` | `"d5b038fb-4b39-41cc-8a10-fba75212180b"` | no |
...
| linux_vm_rg_tags | Module-specific tags merged with the effective root rg_tags and applied to Linux VM module resources. | `map(any)` | `{}` | no |
...
| location | Azure region for the landing zone. | `string` | `"canadacentral"` | no |
...
| management_tenant_id | Azure tenant ID used by the aliased azurerm.management provider. | `string` | `"d5b038fb-4b39-41cc-8a10-fba75212180b"` | no |
...
| prod_tenant_id | Azure tenant ID used by the aliased azurerm.prod provider. | `string` | `"d5b038fb-4b39-41cc-8a10-fba75212180b"` | no |
| resource_group_name | Optional override for the landing zone resource group name. | `string` | `""` | no |
| rg_tags | 13 default tags. The Environment tag is updated dynamically from the environment variable, and the remaining tags can be overridden with rg_tags. | `map(string)` | <pre>{<br>  "AppSupport Team": "CCOE",<br>  "Application Name": "CCOE INFRA IAC",<br>  "Application Owner": "CCOE",<br>  "Approval Group": "CCOE",<br>  "Business Owner": "CCOE",<br>  "Environment": "Sandbox",<br>  "Infra Availability Classification": "Bronze",<br>  "InfraSupport Team": "CCOE",<br>  "Maintenance Window": "CCOE",<br>  "Project Name": "CCOE INFRA IAC",<br>  "Project Number": "N/A",<br>  "RPO-RTO": "48H/24H",<br>  "Run Cost(Approved Run Budget)-USD": "100"<br>}</pre> | no |
...
| security_tenant_id | Azure tenant ID used by the aliased azurerm.security provider. | `string` | `"d5b038fb-4b39-41cc-8a10-fba75212180b"` | no |
...
| tenant_id | Azure tenant ID used by the root azurerm provider. | `string` | `"d5b038fb-4b39-41cc-8a10-fba75212180b"` | no |
...
| winvm_admin_credentials_key_vault_id | Optional Key Vault resource ID to use specifically for Windows VM admin credential secrets. When set, this takes precedence over the shared winvm_iac_kv value. | `string` | `""` | no |
| winvm_admin_password | Optional local administrator password override passed to the Windows VM module. Leave empty to use the configured Key Vault secret. | `string` | `""` | no |
| winvm_admin_password_secret_name | Optional Key Vault secret name containing the Windows VM admin password. | `string` | `""` | no |
| winvm_admin_username | Optional local administrator username override passed to the Windows VM module. Leave empty to use the configured Key Vault secret. | `string` | `""` | no |
| winvm_admin_username_secret_name | Optional Key Vault secret name containing the Windows VM admin username. | `string` | `""` | no |
| winvm_app_admin_group | AD groups granted Contributor on VM/NIC resources and elevated Windows VM login access. | `list(string)` | `[]` | no |
| winvm_app_env | Environment passed to the Windows VM module. Leave empty to use the landing zone environment. | `string` | `""` | no |
| winvm_app_user_group | AD groups granted Reader on the VM resource and standard Windows VM login access. | `list(string)` | `[]` | no |
| winvm_availability_zones | Availability zones used for Windows VM round-robin placement when zone spread is enabled. | `list(string)` | <pre>[<br>  "1",<br>  "2",<br>  "3"<br>]</pre> | no |
| winvm_data_disk_size_gb | Optional additional data disk size in GB. Set to 0 to skip the extra disk. | `number` | `0` | no |
| winvm_domain | AD domain used by the Windows VM module when domain join is enabled. | `string` | `""` | no |
| winvm_domain_join_user | Domain join user used by the Windows VM module when domain join is enabled. | `string` | `""` | no |
| winvm_enable_custom_script_extension | Whether to attach the Windows CustomScriptExtension bootstrap. | `bool` | `false` | no |
| winvm_enable_defender_performance_mode | Whether the Windows bootstrap may temporarily adjust Microsoft Defender settings during heavy software installation steps. | `bool` | `false` | no |
| winvm_enable_diagnostics | Whether to enable Windows VM diagnostics to Log Analytics. | `bool` | `false` | no |
| winvm_enable_domain_join | Whether the Windows VM module should attempt domain join. Sandbox skips domain join inside the module. | `bool` | `false` | no |
| winvm_enable_entra_login | Whether to enable Microsoft Entra login for Windows. | `bool` | `true` | no |
| winvm_enable_shir | Whether to enable Self-Hosted Integration Runtime bootstrap and Data Factory RBAC wiring. | `bool` | `false` | no |
| winvm_enable_zone_spread | Whether to spread multi-VM Windows deployments across availability zones. | `bool` | `true` | no |
| winvm_iac_kv | Shared Key Vault name containing Windows VM secrets. | `string` | `""` | no |
| winvm_iac_rg | Resource group containing the shared IaC storage account and Key Vault. | `string` | `""` | no |
| winvm_iac_st | Shared storage account name containing Windows VM bootstrap scripts. | `string` | `""` | no |
| winvm_location | The Azure region in which the Windows VM resources will be created. Leave empty to use the landing zone location. | `string` | `""` | no |
| winvm_patch_mode | Windows VM guest patching mode. | `string` | `"AutomaticByPlatform"` | no |
| winvm_public_network_enabled | Whether to create public IPs and NSGs for Windows VM access. | `bool` | `false` | no |
| winvm_resource_group_name | Target resource group name for the Windows VM resources. | `string` | `""` | no |
| winvm_subnet_name | Existing subnet name used for the Windows VM NICs. | `string` | `""` | no |
| winvm_tags | Additional tags applied to Windows VM module resources. | `map(string)` | `{}` | no |
| winvm_vm_count | Number of Windows VMs to create. | `number` | `1` | no |
| winvm_vm_name | Base Windows VM name. The winvm module appends a two-digit instance suffix, so keep this short enough for the 15-character Windows computer-name limit. | `string` | `""` | no |
| winvm_vm_size | Azure VM size for each Windows VM. | `string` | `"Standard_D2s_v3"` | no |
| winvm_vnet_name | Existing virtual network name used for the Windows VM NICs. | `string` | `""` | no |
| winvm_vnet_resource_group_name | Resource group containing the target virtual network. | `string` | `""` | no |
...
| Name | Description |
|------|-------------|
| features | Feature enablement and compact connection details for enabled resources. Resource IDs, tags, firewalls, and empty values are intentionally omitted. |
<!-- END_TF_DOCS -->