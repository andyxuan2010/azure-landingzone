...
| common_tags | Legacy/special-case shared tag map. Root module wiring uses the effective `local.rg_tags` tag set by default. | `map(any)` | <pre>{<br>  "AppSupport Team": "CCOE",<br>  "Application Name": "CCOE INFRA IAC",<br>  "Application Owner": "CCOE",<br>  "Approval Group": "CCOE",<br>  "Business Owner": "CCOE",<br>  "Infra Availability Classification": "Bronze",<br>  "InfraSupport Team": "CCOE",<br>  "Maintenance Window": "CCOE",<br>  "Project Name": "CCOE INFRA IAC",<br>  "Project Number": "N/A",<br>  "RPO-RTO": "48H/24H",<br>  "Run Cost(Approved Run Budget)-USD": "100"<br>}</pre> | no |
...
| environment | Environment suffix used in generated names. Also drives the effective `Environment` tag value. | `string` | `"sbx"` | no |
...
| rg_tags | Primary shared tag map for root module resources. The effective local tag set overrides `Environment` from `var.environment` and adds `Workload` from `var.workload`. | `map(string)` | <pre>{<br>  "AppSupport Team": "CCOE",<br>  "Application Name": "CCOE INFRA IAC",<br>  "Application Owner": "CCOE",<br>  "Approval Group": "CCOE",<br>  "Business Owner": "CCOE",<br>  "Environment": "Sandbox",<br>  "Infra Availability Classification": "Bronze",<br>  "InfraSupport Team": "CCOE",<br>  "Maintenance Window": "CCOE",<br>  "Project Name": "CCOE INFRA IAC",<br>  "Project Number": "N/A",<br>  "RPO-RTO": "48H/24H",<br>  "Run Cost(Approved Run Budget)-USD": "100"<br>}</pre> | no |
...
| workload | Short workload or platform identifier used in generated names and the effective `Workload` tag. Must be 4 to 8 characters after trimming whitespace. | `string` | `"lzdemo"` | no |
