locals {
  ...
  feature_flags = {
    ...
    enable_fortigate                       = lookup(var.features, "enable_fortigate", false)
  }
  fortigate_architecture        = lower(trimspace(var.fortigate_deployment_mode)) == "ha" ? "active-passive" : "single"
  fortigate_name_prefix         = trimspace(var.fortigate_name_prefix) != "" ? trimspace(var.fortigate_name_prefix) : "fgt-${local.name_suffix}"
  fortigate_resource_group_name = trimspace(var.fortigate_resource_group_name) != "" ? trimspace(var.fortigate_resource_group_name) : module.resource_group.name
  fortigate_admin_password = (
    trimspace(nonsensitive(var.fortigate_admin_password)) == "" ||
    trimspace(nonsensitive(var.fortigate_admin_password)) == "$(FORTIGATE_ADMIN_PASSWORD)"
  ) ? "" : var.fortigate_admin_password
  fortigate_admin_ssh_public_key = (
    trimspace(nonsensitive(var.fortigate_admin_ssh_public_key)) == "" ||
    trimspace(nonsensitive(var.fortigate_admin_ssh_public_key)) == "$(FORTIGATE_ADMIN_SSH_PUBLIC_KEY)"
  ) ? "" : var.fortigate_admin_ssh_public_key
  fortigate_external_subnet_name     = trimspace(var.fortigate_external_subnet_name) != "" ? trimspace(var.fortigate_external_subnet_name) : "snet-${local.name_suffix}-fortigate-external"
  fortigate_internal_subnet_name     = trimspace(var.fortigate_internal_subnet_name) != "" ? trimspace(var.fortigate_internal_subnet_name) : "snet-${local.name_suffix}-fortigate-internal"
  fortigate_external_subnet_prefixes = var.fortigate_create_dedicated_vnet ? var.fortigate_dedicated_external_subnet_prefixes : var.fortigate_external_subnet_prefixes
  fortigate_internal_subnet_prefixes = var.fortigate_create_dedicated_vnet ? var.fortigate_dedicated_internal_subnet_prefixes : var.fortigate_internal_subnet_prefixes
  fortigate_network_interfaces = length(var.fortigate_network_interfaces) > 0 ? var.fortigate_network_interfaces : [
    {
      name                           = "external"
      role                           = "external"
      subnet_name                    = local.fortigate_external_subnet_name
      address_prefixes               = local.fortigate_external_subnet_prefixes
      primary                        = true
      enabled_in_modes               = toset(["single", "ha"])
      private_ip_address_allocation  = "Static"
      private_ip_addresses_by_suffix = { a = cidrhost(local.fortigate_external_subnet_prefixes[0], 4) }
    ...
    },
    {
      name                           = "internal"
      role                           = "internal"
      subnet_name                    = local.fortigate_internal_subnet_name
      address_prefixes               = local.fortigate_internal_subnet_prefixes
      primary                        = false
      enabled_in_modes               = toset(["single", "ha"])
      private_ip_address_allocation  = "Static"
      private_ip_addresses_by_suffix = { a = cidrhost(local.fortigate_internal_subnet_prefixes[0], 4) }
      enable_ip_forwarding           = true
      accelerated_networking_enabled = false
    }
  ]
  fortigate_module_interfaces = {
    for nic in local.fortigate_network_interfaces : nic.name => {
      role                           = nic.role
      subnet_name                    = nic.subnet_name
      address_prefixes               = try(nic.address_prefixes, [])
  ...
  enabled_automation_accounts = local.feature_flags.enable_automation_accounts ? {
    ...
    key => merge(automation_account, {
      ...
      private_dns_zone_id_effective = try(trimspace(automation_account.private_dns_zone_id), "") != "" ? trimspace(automation_account.private_dns_zone_id) : (
        try(automation_account.private_endpoint_enabled, false) && contains(local.private_dns_zone_names_effective, "privatelink.azure-automation.net") ? try(module.private_dns[0].zone_ids["privatelink.azure-automation.net"], "") : ""
      )
    })
