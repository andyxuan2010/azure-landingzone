...
module "private_dns" {
  count  = length(keys(local.private_dns_zones)) > 0 ? 1 : 0
...
module "fortigate" {
  ...
  admin_password       = local.fortigate_admin_password
  ...
  admin_ssh_source_address_prefixes   = var.fortigate_admin_ssh_source_address_prefixes
  network_security_rules              = var.fortigate_network_security_rules
  interfaces                          = local.fortigate_module_interfaces

  tags = local.rg_tags

  depends_on = [module.hub_virtual_network, module.keyvault]
}
...
module "app_registration_appservice" {
  ...
  web_implicit_grant_id_token_issuance_enabled = local.app_service_platform_auth_enabled
  create_service_principal                     = true
  create_client_secret                         = var.app_registration_create_client_secret
  add_current_caller_as_owner                  = false
  key_vault_id                                 = var.app_registration_create_client_secret ? module.keyvault.id : null
...
module "app_service" {
  ...
  app_settings = merge(
    ...
    each.key == "python" ? {
      FLASK_SECRET_KEY = var.app_service_app_secret_key
    } : {},
    each.key == "node" ? {
      SESSION_SECRET = var.app_service_app_secret_key
    } : {},
  ...
  health_check_eviction_time_in_min = (
    try(each.value.health_check_path, null) != null
    ? coalesce(try(each.value.health_check_eviction_time_in_min, null), 5)
    : null
  )
  custom_hostname_bindings = each.value.custom_hostname_bindings
  ...
  deployment_center_repo_url                 = try(each.value.deployment_center_repo_url, null)
...
module "azure_ai_search" {
  ...
  private_dns_zone_id = trimspace(var.azure_ai_search_private_dns_zone_id) != "" ? trimspace(var.azure_ai_search_private_dns_zone_id) : (
    ...
    ? try(module.private_dns[0].zone_ids["privatelink.search.windows.net"], "")
    : ""
  )
...
module "azure_ai_service" {
  ...
  private_dns_zone_id = trimspace(var.azure_ai_service_private_dns_zone_id) != "" ? trimspace(var.azure_ai_service_private_dns_zone_id) : (
    ...
    ? try(module.private_dns[0].zone_ids["privatelink.cognitiveservices.azure.com"], "")
    : ""
  )
...
module "openai" {
  ...
  private_dns_zone_id = trimspace(var.openai_private_dns_zone_id) != "" ? trimspace(var.openai_private_dns_zone_id) : (
    ...
    ? try(module.private_dns[0].zone_ids["privatelink.openai.azure.com"], "")
    : ""
  )
...
module "acr" {
  ...
  private_dns_zone_id = trimspace(var.acr_private_dns_zone_id) != "" ? trimspace(var.acr_private_dns_zone_id) : (
    ...
    ? try(module.private_dns[0].zone_ids["privatelink.azurecr.io"], "")
    : ""
  )
...
module "databricks" {
  ...
  private_dns_zone_ids = length(var.databricks_private_dns_zone_ids) > 0 ? var.databricks_private_dns_zone_ids : compact([
    ...
    ? try(module.private_dns[0].zone_ids["privatelink.azuredatabricks.net"], "")
    : ""
  ])
...
module "adf_basic" {
  ...
  private_dns_zone_id                  = trimspace(var.adf_private_dns_zone_id) != "" ? var.adf_private_dns_zone_id : (var.adf_enable_private_endpoint ? try(module.private_dns[0].zone_ids["privatelink.datafactory.azure.net"], "") : "")
