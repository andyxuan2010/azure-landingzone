...
module "automation_account" {
  ...
  # Use the effective RG tag set here; keep per-account tags small because the child module adds its own module tag.
...
module "winvm_basic" {
  ...
  source = "git::https://dev.azure.com/CCOE-Azure/IaC/_git/template//modules/winvm?ref=main"
  ...
  tags                       = merge(local.module_tags, var.winvm_tags)
...
module "linux_vm_basic" {
  ...
  inherit_resource_group_tags = false
  tags                        = merge(local.module_tags, var.linux_vm_rg_tags)
...
module "aks" {
  ...
  tags = merge(local.module_tags, var.aks_tags)
