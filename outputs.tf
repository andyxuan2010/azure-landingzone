...
output "acr_id" {
  value       = local.feature_flags.enable_acr ? try(module.acr[0].id, null) : null
  description = "Azure Container Registry resource ID when the ACR module is enabled."
}

output "acr_name" {
  value       = local.feature_flags.enable_acr ? try(module.acr[0].name, null) : null
  description = "Azure Container Registry name when the ACR module is enabled."
}

output "acr_login_server" {
  value       = local.feature_flags.enable_acr ? try(module.acr[0].login_server, null) : null
  description = "Azure Container Registry login server when the ACR module is enabled."
}

output "acr_private_endpoint_id" {
  value       = local.feature_flags.enable_acr ? try(module.acr[0].private_endpoint_id, null) : null
  description = "Azure Container Registry private endpoint resource ID when private endpoint support is enabled."
}
