...
# 13 default tags. The Environment tag is updated dynamically from the environment variable,
# and the remaining tags can be overridden with rg_tags.
variable "rg_tags" {
  type = map(string)
...
variable "workload" {
  ...
  validation {
    condition     = length(trimspace(var.workload)) >= 4 && length(trimspace(var.workload)) <= 8
    error_message = "workload must be 4 to 8 characters."
  }
}
...
variable "environment" {
  ...
  validation {
    ...
    error_message = "environment must be one of: dev, development, sbx, sandbox, prod, production, stg, stage, staging, qa, test, poc, demo, or pentest."
  }
}
...
variable "tenant_id" {
  description = "Azure tenant ID used by the root azurerm provider."
  type        = string
  default     = "d5b038fb-4b39-41cc-8a10-fba75212180b"
}
...
variable "prod_tenant_id" {
  description = "Azure tenant ID used by the aliased azurerm.prod provider."
  type        = string
  default     = "d5b038fb-4b39-41cc-8a10-fba75212180b"
}
...
variable "identity_tenant_id" {
  description = "Azure tenant ID used by the aliased azurerm.identity provider."
  type        = string
  default     = "d5b038fb-4b39-41cc-8a10-fba75212180b"
}
...
variable "management_tenant_id" {
  description = "Azure tenant ID used by the aliased azurerm.management provider."
  type        = string
  default     = "d5b038fb-4b39-41cc-8a10-fba75212180b"
}
...
variable "connectivity_tenant_id" {
  description = "Azure tenant ID used by the aliased azurerm.connectivity provider."
  type        = string
  default     = "d5b038fb-4b39-41cc-8a10-fba75212180b"
}
...
variable "security_tenant_id" {
  description = "Azure tenant ID used by the aliased azurerm.security provider."
  type        = string
  default     = "d5b038fb-4b39-41cc-8a10-fba75212180b"
}
...
variable "enable_databricks" {
  description = "Enable the Azure Databricks workspace module in the landing zone."
  type        = bool
  default     = false
}
...
variable "databricks_root_dbfs_customer_managed_key" {
  description = "Optional customer-managed key configuration for Databricks root DBFS."
  type        = any
  default     = null
}

variable "databricks_enhanced_security_compliance" {
  description = "Optional enhanced security and compliance settings for Databricks."
  type        = any
  default     = null
}
...
variable "databricks_access_connector_role_assignments" {
  description = "Role assignments for the Databricks access connector identity."
  type        = any
  default     = {}
}

variable "databricks_custom_parameters" {
  description = "Optional Databricks custom parameters for VNet injection, no-public-IP, storage, and ML linkage."
  type        = any
  default     = null
}

variable "databricks_private_endpoint_subnet_id" {
  description = "Optional subnet ID for Databricks private endpoints. Defaults to the landing zone private endpoint subnet."
  type        = string
  default     = ""
}
...
variable "databricks_log_analytics_workspace_id" {
  description = "Optional Log Analytics workspace ID for Databricks diagnostics."
  type        = string
  default     = ""
}
...
variable "databricks_role_assignments" {
  description = "Additional Databricks workspace role assignments."
  type        = any
  default     = {}
}
...
variable "fortigate_external_subnet_name" {
  description = "Optional name for the FortiGate external subnet in the hub VNet."
  type        = string
  default     = ""
}

variable "fortigate_create_dedicated_vnet" {
  description = "Whether the FortiGate module creates a dedicated VNet. When false, it creates the FortiGate subnets in the shared hub VNet."
  type        = bool
  default     = false
}

variable "fortigate_virtual_network_name" {
  description = "Optional dedicated FortiGate VNet name. Used only when fortigate_create_dedicated_vnet is true."
  type        = string
  default     = ""
}

variable "fortigate_virtual_network_address_space" {
  description = "Address space for a dedicated FortiGate VNet. The FortiGate subnet prefixes must fall within this space."
  type        = list(string)
  default     = ["10.168.32.0/22"]
}

variable "fortigate_dedicated_external_subnet_prefixes" {
  description = "Address prefixes for the external subnet when FortiGate uses a dedicated VNet."
  type        = list(string)
  default     = ["10.168.32.0/24"]
}

variable "fortigate_dedicated_internal_subnet_prefixes" {
  description = "Address prefixes for the internal subnet when FortiGate uses a dedicated VNet."
  type        = list(string)
  default     = ["10.168.33.0/24"]
}

variable "fortigate_external_subnet_prefixes" {
  description = "Address prefixes for the FortiGate external subnet in the hub VNet."
  type        = list(string)
  default     = ["10.167.34.0/24"]

  validation {
    condition     = length(var.fortigate_external_subnet_prefixes) > 0 && alltrue([for cidr in var.fortigate_external_subnet_prefixes : can(cidrhost(cidr, 4))])
    error_message = "fortigate_external_subnet_prefixes must contain at least one valid CIDR block."
  }
}

variable "fortigate_internal_subnet_name" {
  description = "Optional name for the FortiGate internal subnet in the hub VNet."
  type        = string
  default     = ""
}

variable "fortigate_internal_subnet_prefixes" {
  description = "Address prefixes for the FortiGate internal subnet in the hub VNet."
  type        = list(string)
  default     = ["10.167.35.0/24"]

  validation {
    condition     = length(var.fortigate_internal_subnet_prefixes) > 0 && alltrue([for cidr in var.fortigate_internal_subnet_prefixes : can(cidrhost(cidr, 4))])
    error_message = "fortigate_internal_subnet_prefixes must contain at least one valid CIDR block."
  }
}

variable "fortigate_deployment_mode" {
  description = "FortiGate deployment mode. single creates one VM; ha creates two VMs."
  type        = string
  default     = "single"
...
variable "fortigate_license_type" {
  description = "FortiGate licensing model marker. The Marketplace image and plan must match this value."
  type        = string
  default     = "byol"
...
variable "fortigate_name_prefix" {
  description = "Optional name prefix for FortiGate VM and NIC resources."
  type        = string
  default     = ""
}

variable "fortigate_resource_group_name" {
  description = "Optional existing resource group for FortiGate compute resources. Defaults to the landing zone resource group."
  type        = string
  default     = ""
}
...
variable "fortigate_zone" {
  description = "Optional availability zone for a single FortiGate VM."
  type        = string
  default     = ""
}
...
variable "fortigate_admin_password" {
  description = "Optional FortiGate administrator password. Supply through a secret variable."
  type        = string
  default     = ""
  sensitive   = true
}
...
variable "fortigate_admin_ssh_public_key" {
  description = "Optional SSH public key for FortiGate administration. When empty, linux_vm_admin_ssh_key is used."
  type        = string
  default     = ""
  sensitive   = true
}
...
variable "fortigate_management_access_model" {
  description = "Private management access model recorded on FortiGate resources."
  type        = string
  default     = "private"
...
variable "fortigate_marketplace_plan" {
  description = "Marketplace plan for the selected FortiGate image."
  type = object({
    name      = string
    product   = string
    publisher = string
...
variable "fortigate_network_security_group_name" {
  description = "Optional name for the NSG associated with FortiGate subnets."
  type        = string
  default     = ""
}

variable "fortigate_admin_ssh_source_address_prefixes" {
  ...
  validation {
    condition = alltrue([
      for value in var.fortigate_admin_ssh_source_address_prefixes :
      can(cidrhost(strcontains(trimspace(value), "/") ? trimspace(value) : "${trimspace(value)}/32", 0))
    ])
    error_message = "fortigate_admin_ssh_source_address_prefixes must contain valid IPv4 addresses or CIDR prefixes."
  }
}

variable "fortigate_network_security_rules" {
  description = "NSG rules applied to the FortiGate subnets."
  type = map(object({
    priority                     = number
    direction                    = string
    access                       = string
...
variable "fortigate_network_interfaces" {
  description = "Optional FortiGate NIC definitions. Empty uses external and internal NICs with static .4 addresses."
  type = list(object({
    name                           = string
    role                           = string
    subnet_name                    = string
...
variable "app_services" {
  ...
  type = map(object({
    ...
    custom_hostname_bindings = optional(map(object({
      hostname  = string
    ...
    deployment_center_repo_url                 = optional(string)
  ...
  validation {
    condition = alltrue([
      ...
      try(app_service.health_check_eviction_time_in_min, null) == null ||
      try(app_service.health_check_path, null) != null
    ])
    error_message = "Each app_services entry must set health_check_path when health_check_eviction_time_in_min is provided."
  }

  validation {
    condition = alltrue([
      for _, app_service in var.app_services :
      try(startswith(app_service.health_check_path, "/"), true)
    ])
    error_message = "Each app_services health_check_path must start with '/'."
  }
  ...
  validation {
    condition     = alltrue([for _, app_service in var.app_services : contains(["run_from_package", "deployment_center", "zip_deploy_with_build", "external_zip_deploy"], try(app_service.deployment_method, "run_from_package"))])
    error_message = "Each app_services entry deployment_method must be one of: run_from_package, deployment_center, zip_deploy_with_build, external_zip_deploy."
  }
  ...
  validation {
    condition = alltrue([
      ...
      !(try(app_service.deployment_method, "run_from_package") == "deployment_center" || try(app_service.deployment_center_enabled, false)) || (
        try(trimspace(app_service.deployment_center_repo_url), "") != "" || (
    ...
    error_message = "When deployment_method = \"deployment_center\" (or legacy deployment_center_enabled = true), set deployment_center_repo_url or all three Azure Repos organization, project, and repository fields for that app_services entry."
  }
}
...
variable "app_service_app_secret_key" {
  ...
  type        = string
  default     = "replace-with-a-random-secret-32-long"
  sensitive   = true
}
...
variable "enable_winvm" {
  type        = bool
  description = "Whether to provision the Windows VM jumpbox module."
  default     = false
}

variable "winvm_location" {
  type        = string
  description = "The Azure region in which the Windows VM resources will be created. Leave empty to use the landing zone location."
  default     = ""
}

variable "winvm_app_env" {
  type        = string
  description = "Environment passed to the Windows VM module. Leave empty to use the landing zone environment."
  default     = ""
}

variable "winvm_vm_name" {
  type        = string
  description = "Base Windows VM name. The winvm module appends a two-digit instance suffix, so keep this short enough for the 15-character Windows computer-name limit."
  default     = ""
}

variable "winvm_vm_count" {
  type        = number
  description = "Number of Windows VMs to create."
  default     = 1
}

variable "winvm_vm_size" {
  type        = string
  description = "Azure VM size for each Windows VM."
  default     = "Standard_D2s_v3"
}

variable "winvm_data_disk_size_gb" {
  type        = number
  description = "Optional additional data disk size in GB. Set to 0 to skip the extra disk."
  default     = 0
}

variable "winvm_enable_zone_spread" {
  type        = bool
  description = "Whether to spread multi-VM Windows deployments across availability zones."
  default     = true
}

variable "winvm_availability_zones" {
  type        = list(string)
  description = "Availability zones used for Windows VM round-robin placement when zone spread is enabled."
  default     = ["1", "2", "3"]
}

variable "winvm_admin_username" {
  type        = string
  description = "Optional local administrator username override passed to the Windows VM module. Leave empty to use the configured Key Vault secret."
  default     = ""
}

variable "winvm_admin_password" {
  type        = string
  description = "Optional local administrator password override passed to the Windows VM module. Leave empty to use the configured Key Vault secret."
  default     = ""
  sensitive   = true
}

variable "winvm_admin_credentials_key_vault_id" {
  type        = string
  description = "Optional Key Vault resource ID to use specifically for Windows VM admin credential secrets. When set, this takes precedence over the shared winvm_iac_kv value."
  default     = ""
}

variable "winvm_admin_username_secret_name" {
  type        = string
  description = "Optional Key Vault secret name containing the Windows VM admin username."
  default     = ""
}

variable "winvm_admin_password_secret_name" {
  type        = string
  description = "Optional Key Vault secret name containing the Windows VM admin password."
  default     = ""
}

variable "winvm_iac_rg" {
  type        = string
  description = "Resource group containing the shared IaC storage account and Key Vault."
  default     = ""
}

variable "winvm_iac_kv" {
  type        = string
  description = "Shared Key Vault name containing Windows VM secrets."
  default     = ""
}

variable "winvm_iac_st" {
  type        = string
  description = "Shared storage account name containing Windows VM bootstrap scripts."
  default     = ""
}

variable "winvm_resource_group_name" {
  type        = string
  description = "Target resource group name for the Windows VM resources."
  default     = ""
}

variable "winvm_subnet_name" {
  type        = string
  description = "Existing subnet name used for the Windows VM NICs."
  default     = ""
}

variable "winvm_vnet_resource_group_name" {
  type        = string
  description = "Resource group containing the target virtual network."
  default     = ""
}

variable "winvm_vnet_name" {
  type        = string
  description = "Existing virtual network name used for the Windows VM NICs."
  default     = ""
}

variable "winvm_public_network_enabled" {
  type        = bool
  description = "Whether to create public IPs and NSGs for Windows VM access."
  default     = false
}

variable "winvm_enable_entra_login" {
  type        = bool
  description = "Whether to enable Microsoft Entra login for Windows."
  default     = true
}

variable "winvm_enable_domain_join" {
  type        = bool
  description = "Whether the Windows VM module should attempt domain join. Sandbox skips domain join inside the module."
  default     = false
}

variable "winvm_domain" {
  type        = string
  description = "AD domain used by the Windows VM module when domain join is enabled."
  default     = ""
}

variable "winvm_domain_join_user" {
  type        = string
  description = "Domain join user used by the Windows VM module when domain join is enabled."
  default     = ""
}

variable "winvm_enable_custom_script_extension" {
  type        = bool
  description = "Whether to attach the Windows CustomScriptExtension bootstrap."
  default     = false
}

variable "winvm_enable_defender_performance_mode" {
  type        = bool
  description = "Whether the Windows bootstrap may temporarily adjust Microsoft Defender settings during heavy software installation steps."
  default     = false
}

variable "winvm_enable_shir" {
  type        = bool
  description = "Whether to enable Self-Hosted Integration Runtime bootstrap and Data Factory RBAC wiring."
  default     = false
}

variable "winvm_enable_diagnostics" {
  type        = bool
  description = "Whether to enable Windows VM diagnostics to Log Analytics."
  default     = false
}

variable "winvm_patch_mode" {
  type        = string
  description = "Windows VM guest patching mode."
  default     = "AutomaticByPlatform"
}

variable "winvm_app_user_group" {
  type        = list(string)
  description = "AD groups granted Reader on the VM resource and standard Windows VM login access."
  default     = []
}

variable "winvm_app_admin_group" {
  type        = list(string)
  description = "AD groups granted Contributor on VM/NIC resources and elevated Windows VM login access."
  default     = []
}

variable "winvm_tags" {
  type        = map(string)
  description = "Additional tags applied to Windows VM module resources."
  default     = {}
}
...
variable "linux_vm_rg_tags" {
  type        = map(any)
  description = "Module-specific tags merged with the effective root rg_tags and applied to Linux VM module resources."
  default     = {}
}
